...
function Resumen() {
  ...
  useEffect(() => {
    if (orderId) {
      ...
      fetch(`https://greencart-backend-085d.onrender.com/pedidos/${orderId}`)
        .then(res => {
  ...
  const descargarPDF = async (orderId) => {
    try {
      const response = await fetch(`https://greencart-backend-085d.onrender.com/pedidos/${orderId}/pdf`, {
        method: "GET",
        headers: {
